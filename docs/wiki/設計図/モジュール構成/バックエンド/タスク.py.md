...
- [タスク検証エラー](#タスク検証エラー) を受け取ったアプリ全体の例外ハンドラが本モデルへ変換して返す（[タスク検証エラーハンドラ](./エラーハンドリング.py.md#タスク検証エラーハンドラ)）
...
- HTTP ステータスへの変換はアプリ全体の例外ハンドラが担い、本ファイルでは `HTTPException` を直接送出しない（[エラーハンドリング](./エラーハンドリング.py.md)）
