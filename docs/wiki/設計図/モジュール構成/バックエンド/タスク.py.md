...
| 型 | 説明 | 補足 |
| --- | --- | --- |
| `Task` | 更新後のタスク | 同じタスクを `store` にも反映する |
...
4. タイトルと本文を差し替えたタスクを `store` に書き戻し、そのタスクを返す
...
| --- | --- | --- | --- | --- | --- | --- |
| `test_update_task` | 正常 | タイトルと本文を更新 | `t1` が登録済みのストア | なし | 戻り値のタイトルと本文が更新後の値になっている・`store` の該当タスクも更新後の値になっている | - |
| `test_update_task_when_content_omitted` | 正常 | 本文の省略 | `content` を渡さない | なし | 戻り値の本文が空文字になる | デフォルト値の分岐 |
