...
対応テストファイル: `tests/tasks/test_service.py`
...
| 共通 | サービス | `src/tasks/service.py` | 関数 | [`get_task`](#タスク取得) | ストアからタスクを 1 件取得する | `update_task` から利用 |
...
1. `store` に `task_id` が存在するか確認する（無ければ `TaskNotFoundError`）
2. 該当タスクを返す
...
なし（未登録 ID の分岐は `update_task` の `test_update_task_when_task_missing` で網羅）
