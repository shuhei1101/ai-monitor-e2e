...
| タイトル | `title` | `str` | ✅ | - | 更新後のタスク名 | 100 文字以内。空文字は許容する |
...
1. `title` を検証する（100 文字超なら `ValidationError`。空文字のときは既存のタイトルを維持して更新を続行する）
...
| 例外名 | 発生条件 | メッセージ | 補足 |
| --- | --- | --- | --- |
| `ValidationError` | `title` が 100 文字超 | `"title は 100 文字以内"` | - |
...
| `test_update_task_when_history_recorded` | 正常 | 更新履歴の記録 | `t1` が登録済みのストア | なし | 更新前のタイトルと本文が履歴に残る | - |
| `test_update_task_when_title_empty` | 異常 | タイトルが空 | `title=""` | なし | `ValidationError` | 例外表「title が空文字」に対応 |
