...
3. 送信文を組み立てて send-keys で送信する（[キー送信](./tmux連携.md#キー送信)。[スナップショット組み立て](#スナップショット組み立て)の出力を添付）
   - 新規セッションの場合、shell に対して `claude --dangerously-skip-permissions "{スキル起動文字列 + スナップショット}"` を送信文にする（[スキル起動文字列](#スキル起動文字列)）
   - 既存セッションの場合、稼働中の claude への入力として再開の定型文を送信文にする
...
1. 全エージェント分の確認 / 処理中ラベルを `labels` から取り出して `Agent` を組み立て、一覧で返す（独立系 5 種は `standalone=True` にする）
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_agents` | 正常 | 全エージェント分の Agent 生成 | `LabelSettings` の全値 | なし | エージェント数分の `Agent` が生成され、確認 / 処理中ラベルと独立系フラグが対応する値になる | - |
...
- `main()` は 設定読込 → 台帳復元 → FastAPI アプリの生成（[アプリ生成](./HTTP受信.md#アプリ生成)）→ `uvicorn.run`（`127.0.0.1:{settings.port}`）で起動する。
