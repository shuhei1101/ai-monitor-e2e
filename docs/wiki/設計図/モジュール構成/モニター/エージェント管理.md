...
| 共通 | エージェント上書き | `shared/settings.py` | データモデル | [`AgentOverride`](#エージェント上書き) | エージェント別のモデル上書き 1 件分 | `Settings.agents` の値 |
| 共通 | エージェント定義 | `features/agents/types.py` | データモデル | [`Agent`](#エージェント定義) | 1 エージェントの静的定義 | frozen dataclass |
| 共通 | デフォルトモデル | `shared/settings.py` | 定数 | [`_DEFAULT_AGENT_MODELS`](#デフォルトモデル) | エージェント名 → 役割別デフォルトモデルの辞書 | 上書きがない場合の適用値 |
...
│   └── settings.py        # Settings / LabelSettings / MonitoredProject / AgentOverride / _DEFAULT_AGENT_MODELS
└── features/
    ├── agents/
    │   ├── types.py       # Agent
...
| エージェント上書き | `agents` | [`dict[str, AgentOverride]`](#エージェント上書き) | 公開 | `{}` | エージェント名 → モデル上書き | `{"implementer": AgentOverride(model="sonnet")}` | 上書きなしは[デフォルトモデル](#デフォルトモデル)を適用 |
...
| `test_settings_when_agents_override` | 正常 | エージェント別モデル上書きの読み込み | `agents.implementer.model: sonnet` を書いた settings.yaml | なし | `settings.agents["implementer"].model == "sonnet"` | - |
...
## エージェント上書き
> 物理名: `AgentOverride`<br>
> 種別: データモデル<br>
> コンテナ: `shared/settings.py`

エージェント別のモデル上書き 1 件分（`pydantic.BaseModel`）。
...
| 論理名 | プロパティ名 | 型 | 可視性 | デフォルト | 説明 | 例 | 補足 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| モデル | `model` | `str` | 公開 | - | 起動時の `claude --model` に渡す値 | `"claude-opus-4-7"` / `"sonnet"` | 完全 ID / 短縮名のどちらでも可・バリデーションはしない |
...
## デフォルトモデル
> 物理名: `_DEFAULT_AGENT_MODELS`<br>
> 種別: 定数<br>
> コンテナ: `shared/settings.py`

エージェント名 → 役割別デフォルトモデルの辞書（`dict[str, str]`）。
`Settings.agents` に上書きがない場合の適用値。
値の一覧は[設定/モニター](../../設定/モニター.md#役割別デフォルトモデル)を参照。
...
- 頭を使う工程（設計・レビュー・判断）は `claude-opus-4-7`、機械的な工程（テスト作成・実装・軽微修正）は `sonnet` を割り当てる
- 全 17 エージェント分のエントリを必ず持つ（欠落は `build_agents` の `KeyError` で気づく）
...
| モデル | `model` | `str` | 公開 | - | 起動時の `claude --model` に渡す値 | `"claude-opus-4-7"` | `Settings.agents[name].model` があればそれ・なければ[デフォルトモデル](#デフォルトモデル) |
...
全エージェントの[エージェント定義](#エージェント定義)を[ラベル設定](#ラベル設定)の値と[エージェント上書き](#エージェント上書き)から組み立てる。
...
| エージェント上書き | `agent_overrides` | [`dict[str, AgentOverride]`](#エージェント上書き) | - | `{}` | エージェント名 → モデル上書き | 未指定のエージェントは[デフォルトモデル](#デフォルトモデル)を適用 |

引数例:

```python
build_agents(labels, agent_overrides=settings.agents)
```
...
1. 全エージェント分の確認 / 処理中ラベルを `labels` から取り出す（独立系 5 種は `standalone=True` にする）
2. モデルを解決する（`agent_overrides.get(name).model` があればそれ・なければ[デフォルトモデル](#デフォルトモデル)の値）
3. `Agent` を組み立て、一覧で返す
...
| 例外名 | 発生条件 | メッセージ | 補足 |
| --- | --- | --- | --- |
| `KeyError` | [デフォルトモデル](#デフォルトモデル)にエージェント名のエントリがない | 該当エージェント名 | エージェント追加時に定数への追記漏れを気づける |
...
| テスト名 | 正常/異常 | 概要 | 条件 | Mock | 期待値 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| `test_build_agents` | 正常 | 全エージェント分の Agent 生成 | `LabelSettings` の全値・`agent_overrides` なし | なし | エージェント数分の `Agent` が生成され、確認 / 処理中ラベルと独立系フラグが対応する値になり、`model` は[デフォルトモデル](#デフォルトモデル)の値になる | - |
| `test_build_agents_when_override` | 正常 | エージェント別モデル上書きの反映 | `agent_overrides={"implementer": AgentOverride(model="sonnet")}` | なし | `implementer` の `Agent.model` が `"sonnet"` になり、その他は[デフォルトモデル](#デフォルトモデル)の値のまま | - |
...
1. プロジェクトごとに open 対象一覧を取得する（[オープン対象一覧](./GitHub連携.md#オープン対象一覧)。周期 1 回・全エージェントで共有）
2. プロジェクト × エージェントの対ごとにポーリングを実行する（[ポーリング](#ポーリング)）
3. クリーンアップ検知を実行する（[intake 自動クローズ](./クリーンアップ.md#intake-自動クローズ)・[epic 一括解放](./クリーンアップ.md#epic-一括解放)・[個別解放](./クリーンアップ.md#個別解放)）
4. 前回 heartbeat から `heartbeat_interval_sec` 以上経過していれば[タイムアウト回収](./クリーンアップ.md#タイムアウト回収)を実行し、heartbeat 時刻を更新する
...
- `main()` は 設定読込 → 台帳復元 → `build_agents(labels, agent_overrides=settings.agents)` → FastAPI アプリの生成（[アプリ生成](./HTTP受信.md#アプリ生成)）→ `uvicorn.run`（`127.0.0.1:{settings.port}`）で起動する。
