...
| 共通 | エージェント名一覧 | `features/agents/service.py` | 定数 | [`_AGENT_NAMES`](#エージェント名一覧) | 全 17 エージェント名のタプル | 設定の網羅性検証・build_agents のループの出所 |
...
│   └── settings.py        # Settings / LabelSettings / MonitoredProject / AgentModel
└── features/
    ├── agents/
    │   ├── types.py       # Agent
...
> 物理名: `_AGENT_NAMES`<br>
> 種別: 定数<br>
> コンテナ: `features/agents/service.py`
...
- `main()` は 設定読込 → 台帳復元 → `build_agents(labels, agent_models=settings.agents)` → FastAPI アプリの生成（[アプリ生成](./HTTP受信.md#アプリ生成)）→ `uvicorn.run`（`127.0.0.1:{settings.port}`）で起動する。
