...
| `title` | `str` | ✅ | - | 更新後のタスク名 | 1 文字以上 100 文字以内 | SA の機能要件の制約 |
...
| 更新履歴 | 保持しない | SA の機能要件になく、スコープ外の「DB スキーマの変更」に当たるため |
...
| 異常 | 異常系（タイトルが空・ValidationError） | タイトルの空文字検証に失敗 | 単一UC シナリオの異常シナリオに対応 |
