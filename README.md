# タスク管理ツール

紙とメモアプリに散らばったタスクを 1 箇所に集めて、登録・編集・一覧できる個人向けのツール。

## 概要

個人でタスクを持つ人向けに、タスクの登録・編集・一覧を 1 つのツールで完結できるようにする。
未着手のタスクを見失わない状態を作ることが目的。
対象ユーザーは個人 1 名で、同時に複数人が使うことは想定しない。

## 機能

未確定

## システム構成

```mermaid
flowchart LR
  U([ユーザー]) --> BE[バックエンド<br>Python 3.12]
  BE --> STORE[(インメモリストア)]
```

| サブシステム | 役割 | 補足 |
| --- | --- | --- |
| バックエンド | タスクの取得・更新・一覧 | 画面は未実装 |

詳細は [設計図/アーキテクチャ図](./docs/wiki/設計図/アーキテクチャ図.md) を参照。

## ディレクトリ構成

```
ai-monitor-e2e/
├── src/tasks/         # バックエンド（Python 3.12）
├── tests/             # テストコード
├── docs/wiki/         # 設計ドキュメント
└── README.md
```

## セットアップ

**前提:**

- Python 3.12 以上（標準ライブラリのみを使うため、パッケージのインストールは不要）

**手順:**

```bash
git clone https://github.com/shuhei1101/ai-monitor-e2e.git
cd ai-monitor-e2e
```

## 開発

**起動:**

未確定

**テスト:**

実行コマンドは [テスト/テスト実行方法](./docs/wiki/テスト/テスト実行方法.md) を参照。

## ドキュメント

| リソース | 内容 |
| --- | --- |
| [設計ドキュメント](./docs/wiki/) | シナリオ / 設計図 / 規約の全体 |
| [設計図/アーキテクチャ図](./docs/wiki/設計図/アーキテクチャ図.md) | システム全体の構成と外部システム連携 |
| [設計図/非機能要件](./docs/wiki/設計図/非機能要件.md) | 外部制約を踏まえたアプリの決定値 |
| [外部API](./docs/wiki/外部API/) | 連携している外部 API の一覧 |
| [外部ライブラリ](./docs/wiki/外部ライブラリ/) | 採用ライブラリと選定理由の一覧 |
