...
class タスク編集から一覧反映Test(unittest.TestCase):
    def test_normal(self):
        """タスクを編集して保存すると一覧に編集後のタイトルと本文が並ぶ（正常系）。"""
        # 準備
        store = _store()
        # 実行
        ...
        self.assertEqual(tasks[0].title, "新タイトル")
        self.assertEqual(tasks[0].content, "新本文")

    def test_error_when_タイトルが空(self):
        """タイトルを空にして保存すると一覧が編集前の内容のまま変わらない（異常系）。"""
        # 準備
        store = _store()
        # 実行・検証
        ...
        tasks = list_tasks(store)
        self.assertEqual(len(tasks), 1)
        self.assertEqual(tasks[0].title, "旧タイトル")
        self.assertEqual(tasks[0].content, "旧本文")
